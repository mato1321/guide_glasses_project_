...
### D1. 翻譯（無阻塞，約 3 天）⭐ 可立刻開工

- [ ] domain：`Translator` 介面
- [ ] domain：`TranslateUseCase` + Outcome
- [ ] domain：單元測試
- [ ] 建立 `ai/ai-translate` 模組
- [ ] `MlKitTranslator`（`com.google.mlkit:translate`，離線）
- [ ] 語言包下載進度回饋（「正在準備翻譯」）
- [ ] 無 BFF 時的預設目標語言（建議英文）
- [ ] ViewModel 分派 + DI 接線
- [ ] `./gradlew build` 通過
