...
- [x] ~~A0-17 驗證 ASR 辨識準確度~~ → ✅ 「前面有什麼」正確辨識（5.5 秒）。
      過程中修掉兩個坑：`VOICE_RECOGNITION` 音訊來源是啞的、模型輸出簡體
- [ ] A0-19 測更多指令的辨識率（`tools/test_commands.ps1`）。
      已知會誤判：「翻成英文」→「翻层英文」
- [x] ~~A0-20 喚醒詞觸發率~~ → 一般辨識＋字串比對只有 4/8，**換成關鍵詞偵測模型**
      並取消喚醒步驟（直接聽指令）
- [ ] A0-21 測 12 個語音指令各自的偵測率與誤觸率（`偵測到語音指令：` log）
- [ ] A0-22 評估把 OCR／翻譯／人臉的運算卸載到手機（音訊資料量小，卸載划算，
      與視覺的結論相反；但安全播報不可依賴手機）
- [x] ~~A8 人臉辨識實測~~ → ✅ 同步 2 人 6 張照片成功，使用者回報辨識「還行」
...
- [x] ~~A24b 播報音量偏小~~ → ✅ 合成輸出只有 -13.6 dBFS，播放前套 3.5 倍增益（實測 +11dB）
...
- [ ] BFF 後端本身（見 §D5）
- [ ] 眼鏡 AI 實體鍵整合
- [x] ~~喚醒詞~~ → ✅ 「呼叫盲狗」，常駐監聽（`WakeWord` + `startWakeWordListening`）
