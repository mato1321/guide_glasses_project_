> 🔴 **2026-08-08 更新：§7 的待驗證清單多數已有答案。**
> 實測結果見 [`DEVICE_FINDINGS.md`](DEVICE_FINDINGS.md)。
...
| IMU | ✅ **實測：有加速度計 + 陀螺儀，沒有磁力計（無電子羅盤）** | 相機省電、步態輔助。⚠️ **拿不到絕對方位** |
| 連線 | Wi-Fi 6、BT 5.3 | |
| 重量 | 49 g | 長時間配戴的最大優勢 |
...
> ✅ **2026-08-08 已解答的**：Play Services（沒有）、`SpeechRecognizer`（沒有）、
> TTS（綁不上）、GPS（沒有）、電子羅盤（沒有）、相機延遲（930ms）、
> 相機解析度（480×640）、2GB RAM 跑 YOLO ONNX（可以）。
> 全部見 [`DEVICE_FINDINGS.md`](DEVICE_FINDINGS.md)。
>
> ⏳ **仍未驗證**：相機視角校正、連續 2fps 續航、邊充邊用、Device Owner、
> 人臉辨識在眼鏡上的實際表現、OCR 對真實藥袋的品質。

**以下是原始清單。** App 內建「測試相機」「測試感測器」兩個
語音指令可直接確認前兩項。
