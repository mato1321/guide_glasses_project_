...
>
> 最後更新：2026-08-08 ｜ 相關：[`STATUS.md`](STATUS.md)、[`DEVICE_FINDINGS.md`](DEVICE_FINDINGS.md)
>
> 🔴 **2026-08-08 實機推翻了本文件的兩個前提**，詳見
> [`DEVICE_FINDINGS.md`](DEVICE_FINDINGS.md)：
> 1. 「語音一律本機」—— 眼鏡上 **TTS 綁不上、STT 完全不存在**
> 2. 「擷取約 100ms」—— 實測 **930ms**，所有延遲推算都要重算
...
| GPS | 🔴 **實測確認沒有**（宣告有特徵但無 provider） | 有 |
| 電子羅盤（磁力計） | 🔴 **實測確認沒有** —— 拿不到絕對方位 | 有 |
| 相機擷取延遲 | 🔴 **實測 930ms**（估計值是 145ms） | ~100ms |
| TTS / STT | 🔴 **實測皆不可用** | 可用 |
...
> ✅ **A10 已於 2026-08-08 實測解答：眼鏡沒有 GPS。**
> `dumpsys location` 只有 `passive` / `fused`，沒有 `gps` provider，
> 也沒有 GNSS HAL。定位抽象（§5.3）應綁 `PhoneCompanionLocationProvider`。
>
> 🔴 **但還有一個更難的問題：眼鏡也沒有電子羅盤。**
> 即使手機提供座標，`Geo.relativeTurn()` 需要的 `headingDegrees` 仍然拿不到，
> **算不出「往左轉還是往右轉」**。導航難度顯著高於本文件原先的評估。
...
> 現在是 `相機 → 端側 → TTS`，零成本、離線可用。
> ⚠️ 但**實測相機擷取就要 930ms**，「約 100ms」那個數字是錯的。
...
單一 APK 的眼鏡版本已經有 80% 完成度、306 個測試、能跑。手機 companion 是
...
| 階段 | 做什麼 | 為什麼是這個順序 |
|---|---|---|
| ~~0~~ | ~~實機驗證~~ | ✅ **已完成**（2026-08-08）。A9/A10/A3/A4 都有答案了，見 `DEVICE_FINDINGS.md` |
...
| **5** | 手機 companion —— **A10 已確認沒有 GPS，需要做** | 範圍限制在「定位 + 網路中繼 + 註冊 UI」。⚠️ 但沒有羅盤，導航仍不完整 |
...
| ~~STT / TTS~~ | 🔴 **眼鏡上兩者皆不可用**，見 `DEVICE_FINDINGS.md` §3、§4 |
| 翻譯 ML Kit | 語言包下載後離線 |
| 意圖路由第一層 | 「停」不該等雲端 |
...
| 端側 OCR / 翻譯 / 障礙物可運作 | ✅ **2026-08-08 Rokid Glasses 實測通過** |
| 端側人臉可運作 | 🟡 程式完成，眼鏡上尚未實測 |
| 眼鏡沒有 GPS | ✅ **實測確認** |
| 眼鏡沒有電子羅盤 | ✅ **實測確認** |
