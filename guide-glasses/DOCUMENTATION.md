...
> 🔴 **2026-08-08 實機更新**：眼鏡上**語音完全不可用**（TTS 綁不上、
> STT 不存在），且沒有 Play Services、沒有 GPS、沒有電子羅盤，
> 相機擷取要 930ms。本文件多處延遲估算已過時。
> **實機事實見 [`../docs/DEVICE_FINDINGS.md`](../docs/DEVICE_FINDINGS.md)。**
...
> 相關文件：[現況快照](../docs/STATUS.md)｜[待辦清單](../docs/TASKS.md)｜[實作規劃](../docs/IMPLEMENTATION_PLAN.md)｜[技術筆記](../docs/TECHNICAL_NOTES.md)｜[路線圖](../docs/ROADMAP.md)
...
>
> ⬆️ 這是 **2026-08-08 在 Rokid Glasses 上的實測值**。930ms 遠高於原本
> 估計的 145ms —— 障礙物的延遲預算是 <300ms，光擷取就超支 3 倍。
...
| 🔴 **確認眼鏡沒有 GPS，也沒有電子羅盤** | 導航需手機提供座標，且**算不出朝向** | 實測完成，見 `DEVICE_FINDINGS.md` §9、§17 |
| 🔴 **確認眼鏡沒有 Play Services、無 STT、TTS 綁不上** | **目前無法用語音操作** | 見 `DEVICE_FINDINGS.md` §2–§4 |
